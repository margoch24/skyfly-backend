...
class TestGetPastTickets(TestInitializer):
    __title__ = "GET /past-tickets"

    def test_get_past_tickets_successful(self):
        headers = get_headers()
        ...
        response = app.get("/past-tickets", headers=headers)
        ...
        for index, ticket in enumerate(tickets):
            ...
            self.assertLess(ticket.get("flight").get("arrival"), current_milli_time())
    ...
    def test_get_past_tickets_pagination(self):
        headers = get_headers()
        user_id = headers.get("user_id")
        ...
        # Page 0
        response = app.get(f"/past-tickets?limit={limit}&page=0", headers=headers)
        parsed_response = ParsedResponse(response)
        ...
        # Page 1
        response1 = app.get(f"/past-tickets?limit={limit}&page=1", headers=headers)
        ...
        # Page 2
        response2 = app.get(f"/past-tickets?limit={limit}&page=2", headers=headers)
    ...
    def test_passing_wrong_params(self):
        ...
        response = app.get(f"/past-tickets{query}", headers=headers)
        parsed_response = ParsedResponse(response)
    ...
    def test_passing_empty_headers(self):
        response = app.get("/past-tickets", headers={})
        parsed_response = ParsedResponse(response)
    ...
    def test_passing_empty_auth(self):
        ...
        response = app.get("/past-tickets", json={}, headers=headers)
        parsed_response = ParsedResponse(response)
